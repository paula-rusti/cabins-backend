...
class PhotoRepository(AbstractPhotoRepository):
    ...
    def add(self, photo: dto_models.PhotoCreate):
        ...
        if photo.principal:
            self.db.query(orm_models.Photo). \
                filter_by(cabin_id=photo.cabin_id, principal=True).update({'principal': False})
            self.db.commit()
        self.db.add(orm_models.Photo(**photo.dict()))
        self.db.commit()

    def get_by_id(self, id):
        return self.db.query(orm_models.Photo).filter(orm_models.Photo.id == int(id)).first()
    ...
    def get_photos_of_cabin(self, cabin_id):
        ...
        results = self.db.query(orm_models.Photo).filter(orm_models.Photo.cabin_id == int(cabin_id)).all()
